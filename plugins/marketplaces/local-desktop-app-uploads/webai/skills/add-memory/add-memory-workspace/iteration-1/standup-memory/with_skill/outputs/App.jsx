...
You also have access to the user's previous standups from memory. If you notice recurring blockers or patterns — for example, the same blocker mentioned multiple times — point it out with a brief note at the end under **Patterns Noticed** (only include this section if there is something worth flagging).

Do not add any preamble or closing remarks beyond the format above.`;
...
const styles = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .btn-clear-history:disabled { opacity: 0.4; cursor: not-allowed; }
  ...
  .history-turn-user .history-turn-text {
    background: var(--accent);
    color: #fff;
    border-color: transparent;
  }

  .history-turn-assistant .history-turn-text strong {
    font-weight: 700;
  }
...
export default function App() {
  ...
  // Scroll history to bottom when turns update
  useEffect(() => {
...
${blockers.trim() || 'None'}`;
    ...
    const userTurn = { role: 'user', text: prompt, at: new Date().toISOString() };
    ...
    try {
      ...
      setTurns(prev => {
        const next = [...prev];
        next[next.length - 1] = { ...assistantTurn, text: errText };
        return next;
      });
    } finally {
  ...
  // Only show assistant turns in the history panel (formatted standup responses)
      ...
      <main className="main">
        ...
        <div className="history-section">
          <div className="history-header">
            <span className="history-title">📅 Past Standups</span>
            <button
              className="btn-clear-history"
              onClick={handleClearHistory}
              disabled={isGenerating || assistantTurns.length === 0}
            >
              Clear history
            </button>
          </div>
          ...
          {assistantTurns.length === 0 ? (
            <div className="history-empty">
              No standup history yet — generate your first one above.
            </div>
          ) : (
            <div className="history-list">
              {assistantTurns.map((turn, i) => (
                <div
                  key={turn.turnId ?? `assistant-${i}`}
                  className="history-turn history-turn-assistant"
                >
                  <span className="history-turn-role">
                    AI · {formatTurnTime(turn.at)}
                  </span>
                  <div className="history-turn-text">
                    {renderOutput(turn.text || '…')}
                  </div>
                </div>
              ))}
